...
| `onAllWavesComplete()` | Launch VictoryUIScene | All waves cleared |
...
```typescript
enum CellType { BUILDABLE = 0, PATH = 1, BLOCKED = 2, SPAWN = 3, EXIT = 4 }
...
interface PathPoint { gridX: number; gridY: number }
```
...
- **Tower Slots**: Call `drawTowerSlots()` in `createEnvironment()` and assign result to `this.towerSlotGroup`. Slots are **hidden by default** and automatically shown when the player selects a tower type (placement mode). Uses `tower_slot` texture if available; falls back to rounded-rect graphic. This replaces the need for `drawBuildableMarkers()`.
...
- **Floating Text**: Call `showFloatingText()` in hooks like `onEnemyKilled()` for reward popups.
...
- **Tower Fire Animation**: Towers automatically play a subtle scale pulse when firing. Override `playFireAnimation()` for custom effects.
