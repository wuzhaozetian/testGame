...
export abstract class BaseTDScene extends Phaser.Scene {
  ...
  private initializeSystems(): void {
    ...
    this.events.on('obstacleDestroyed', (obstacle: BaseObstacle, reward: number) => {
      if (reward > 0) {
        this.economyManager.earn(reward);
      }
  ...
  private updateProjectiles(): void {
    const projectiles = this.projectilesGroup.getChildren() as Phaser.Physics.Arcade.Sprite[];
  ...
  private checkEndConditions(): void {
    ...
    if (this.waveManager.isAllWavesComplete && !this.isVictory && !this.isGameOver) {
      this.isVictory = true;
  ...
  private handleLeftClick(pointer: Phaser.Input.Pointer): void {
    ...
    // Clear selection after placing — no more preview following mouse
    this.cancelTowerSelection();
  }
  ...
  private setTowerSlotsVisible(visible: boolean): void {
    ...
    for (const child of children) {
      (child as Phaser.GameObjects.GameObject & { setVisible: (v: boolean) => void }).setVisible(visible);
    }
  }
  ...
  private updatePlacementPreview(pointer: Phaser.Input.Pointer): void {
    ...
    this.rangePreview = createRangeIndicator(
      this, world.x, world.y, towerConfig.range,
