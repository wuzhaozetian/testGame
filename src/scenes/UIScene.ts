...
export default class UIScene extends Phaser.Scene {
  ...
  createDOMUI(): void {
    const towerButtonsHTML = this.towerTypes.map(t => `
    ...
    const uiHTML = `
      <div id="ui-container" class="fixed top-0 left-0 w-full h-full pointer-events-none z-[1000] font-retro">
        ...
        <!-- Wave countdown timer (between waves) - above tower panel to avoid overlap -->
        <div id="wave-timer" class="absolute bottom-32 left-1/2 transform -translate-x-1/2 hidden" style="z-index: 1050;">
  ...
  private updateTowerButtonStyles(): void {
    ...
    buttons.forEach((btn) => {
      ...
      if (towerId === this.selectedTowerTypeId) {
        el.style.background = 'linear-gradient(180deg, rgba(40,100,180,0.95) 0%, rgba(30,70,140,0.95) 100%)';
        el.classList.add('border-yellow-400');
        el.classList.remove('border-amber-700');
      } else {
        el.style.background = 'linear-gradient(180deg, rgba(139,90,43,0.95) 0%, rgba(101,67,33,0.95) 100%)';
  ...
  private setupGameEventListeners(): void {
    ...
    gameScene.events.on('livesChanged', (_oldLives: number, newLives: number) => {
      this.lives = newLives;
      this.updateLivesDisplay();
    });
    ...
    gameScene.events.on('waveComplete', (_waveNum: number, _totalWaves: number) => {
      this.showWaveTimer();
    });
