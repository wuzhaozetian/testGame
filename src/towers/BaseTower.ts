...
export class BaseTower extends Phaser.GameObjects.Image {
  ...
  constructor(
    ...
    initScale(this, { x: 0.5, y: 0.5 }, undefined, 96, 0.8, 0.8);
  ...
  private showRangeCircle(): void {
    ...
    this.rangeCircle = createRangeIndicator(
      this.scene, this.x, this.y, this.currentRange,
      this.getRangeCircleColor(), 0.2
    );
  }
  ...
  protected getUpgradeStats(): TowerUpgrade | null {
    if (!this.towerConfig.upgrades || this.towerConfig.upgrades.length === 0) return null;
